...
const activities = [
  {
    ...
    description:
      "Successfully shared 50 portions of vegetable soup with the downtown shelter.",
    time: "2h ago",
  ...
  },
  {
    ...
    description:
      "Rescued 30 loaves of bread and distributed to local families.",
    time: "1d ago",
    icon: <Utensils className="w-5 h-5" />,
    participants: 5,
  },
  ...
  return (
    <main className="min-h-screen w-full max-w-md mx-auto bg-surface flex flex-col pb-32">
      ...
      <div className="flex-1 px-6 pt-10 space-y-10 overflow-y-auto hide-scrollbar">
        ...
        <Card
          padding="lg"
          className="rounded-[2.5rem] shadow-xl border border-border flex flex-col items-center text-center"
        >
          <div className="relative mb-6">
            <div className="w-28 h-28 rounded-full border-4 border-surface shadow-xl overflow-hidden grayscale">
              <Image
                src={
                  user?.avatarUrl || "https://picsum.photos/seed/alex/200/200"
                }
                alt={user?.name || "User"}
                width={112}
                height={112}
          ...
          <h2 className="text-2xl font-black mb-1 text-primary">
            {user?.name || "Guest User"}
          </h2>
          <p className="text-sm font-bold text-muted-foreground mb-8">
            @{user?.username || "guest"} • Joined{" "}
        ...
        <section className="space-y-4">
          <h3 className="text-base font-bold px-1 text-primary">
            Badges & Achievements
          </h3>
          ...
          <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-4 -mx-1 px-1">
            {activities.map((act) => (
              <Card
                key={act.id}
                padding="lg"
                className="min-w-[260px] rounded-3xl shadow-xl border border-border space-y-4"
              >
                ...
                <div className="flex -space-x-2 grayscale">
                  {[1, 2].map((i) => (
                    ...
                    >
                      <Image
                        src={`https://picsum.photos/seed/u${act.id}${i}/50/50`}
                        alt="User"
                        width={24}
                        height={24}
                        referrerPolicy="no-referrer"
        ...
        <Card
          padding="lg"
          className="rounded-[2.5rem] shadow-xl border border-border space-y-8"
        >
