...
import { CheckCircle2, EyeOff, Eye, Check, X, ArrowRight } from "lucide-react";
  ...
  return (
    <main className="min-h-screen w-full max-w-md mx-auto bg-surface flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <Header
        title=""
        backHref="/"
        className="bg-transparent border-none absolute top-0 left-0 right-0"
      />
      ...
      >
        ...
        {error && (
          <div className="bg-destructive/10 text-destructive text-sm font-medium p-4 rounded-2xl border border-destructive/20 text-center">
            {error}
          </div>
        )}
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-4 pt-4">
            ...
            <button
              className="w-full text-muted-foreground font-bold text-xs uppercase tracking-widest hover:text-primary transition-colors py-2"
              type="button"
            >
              Forgot Password?
            </button>
