    ...
    <main className="min-h-screen w-full max-w-md mx-auto bg-surface flex flex-col">
      ...
      <div className="flex-1 px-6 py-8 flex flex-col items-center">
        <div className="text-center space-y-2 mb-10">
          ...
          <p className="text-sm text-muted-foreground font-medium px-8">Please enter your current password to create a new one.</p>
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="mt-4 space-y-3 px-1">
              <div className="flex gap-1 h-1.5 w-full">
                <div className={`flex-1 rounded-full ${password.length > 0 ? 'bg-primary' : 'bg-muted'}`} />
                <div className={`flex-1 rounded-full ${password.length > 4 ? 'bg-primary' : 'bg-muted'}`} />
                <div className={`flex-1 rounded-full ${password.length > 8 ? 'bg-primary' : 'bg-muted'}`} />
                <div className={`flex-1 rounded-full ${password.length > 10 ? 'bg-primary' : 'bg-muted'}`} />
              </div>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Fair password</p>
              <ul className="space-y-2">
                {requirements.map((req, i) => (
                  <li key={i} className={`flex items-center gap-2 text-xs font-bold ${req.met ? 'text-primary' : 'text-muted-foreground/40'}`}>
