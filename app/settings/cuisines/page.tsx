...
import { Search, Check, Utensils, Apple, Leaf, Coffee, Soup, Pizza } from 'lucide-react';

const cuisines = [
  { id: 'street', name: 'Street Food', description: 'Food trucks, stalls, quick bites', icon: <Utensils className="w-5 h-5" /> },
  { id: 'bakery', name: 'Bakery', description: 'Pastries, bread, desserts', icon: <Apple className="w-5 h-5" /> },
  { id: 'vegan', name: 'Vegan', description: 'Plant-based, cruelty-free', icon: <Leaf className="w-5 h-5" /> },
  { id: 'italian', name: 'Italian', description: 'Pasta, pizza, Mediterranean', icon: <Pizza className="w-5 h-5" /> },
  { id: 'seafood', name: 'Seafood', description: 'Fish, shellfish, oceanic', icon: <Soup className="w-5 h-5" /> },
  { id: 'coffee', name: 'Coffee & Tea', description: 'Cafes, roasteries, beverages', icon: <Coffee className="w-5 h-5" /> },
];
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
                    isSelected ? 'bg-primary text-surface' : 'bg-muted text-primary'
                  }`}>
                    {cuisine.icon}
                  </div>
                  <div className="text-left">
                    <h3 className="font-bold text-sm text-primary">{cuisine.name}</h3>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">{cuisine.description}</p>
                  </div>
                </div>
